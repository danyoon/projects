class StaticPagesController < ApplicationController
  ...
  def index
    ...
    @dates = Array.new
    14.times do |index|
      temp =  @firstDate + index.day
      @dates[index] = temp.strftime("%m/%d/%Y")
    end
    ...
    api = Expedia::Api.new
    @responseGL = api.get_list({:numberOfResults =>"20", :arrivalDate => @firstDateString,:departureDate => @secondDateString,:hotelIDList =>  "131734, 309378", :room1 => "2", :options => "ROOM_RATE_DETAILS"})
    if @responseGL.exception?
    else
      @hotelGL = @responseGL.body['HotelListResponse']['HotelList']['HotelSummary']['hotelId']
      @outputGL = @responseGL.body['HotelListResponse']['HotelList']['HotelSummary']['RoomRateDetailsList']['RoomRateDetails']['RateInfos']['RateInfo']['ChargeableRateInfo']
    end
